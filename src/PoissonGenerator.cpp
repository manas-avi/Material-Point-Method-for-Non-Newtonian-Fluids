...
sPoint PoissonGenerator::GenerateRandomPointAround( const sPoint& P, FLOAT MinDist, PRNG& Generator )
{
	...
	FLOAT Anglez = 2 * 3.141592653589 * R3;

	// the new point is generated around the point (x, y)
	VEC3 V(P.v(0) + Radius * cos(Angleh) * sin(Anglez),
		P.v(1) + Radius * sin(Angleh) * sin(Anglez),
		P.v(2) + Radius * cos(Anglez));
	return sPoint(V);
}
...
std::list<VEC3> PoissonGenerator::GeneratePoissonPointsR(size_t NumPoints,
	...
	if ( MinDist < 0.0f ) {
		...
		MinDist = pow(RectSize(0)*RectSize(1)*RectSize(2)/FLOAT(NumPoints), 1.0/3.0 );
	}
	...
	// HERE grid is just used as a helper function to help the neighbour function
...
std::list<VEC3> PoissonGenerator::GeneratePoissonPointsC(size_t NumPoints,
	...
	if ( MinDist < 0.0f ) {
		//    MinDist = pow( FLOAT(NumPoints), 1.0/3.0 ) / FLOAT(NumPoints);
		MinDist = pow( 4.0/3.0*M_PI*SphereSize*SphereSize*SphereSize/FLOAT(NumPoints), 1.0/3.0 );
	}
	...
	while ( !ProcessList.empty() && SamplePoints.size() < NumPoints )
	{
		...
		{
			sPoint NewPoint = GenerateRandomPointAround( Point, MinDist, Generator );
			bool Fits = NewPoint.IsInSphere(SphereSize);
			if ( Fits && !Grid.IsInNeighbourhood( NewPoint, MinDist, CellSize ) )
			{
				ProcessList.push_back( NewPoint );
				SamplePoints.push_back( NewPoint );
